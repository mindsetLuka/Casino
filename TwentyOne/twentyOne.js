...
function startGame() {
    if (roundOver === true) {
        ...
        for (i = 0; i < 10; i++) {
            if (i === 0 || i === 1) {
                srcId = "pcard" + i
                document.getElementById(srcId).src = "img/green_back.png"
                srcId = "dcard" + i
                document.getElementById(srcId).src = "img/green_back.png"
            } else {
                srcId = "pcard" + i
...
function renderGame() {
    ...
    for (i = 0; i < playerHand.length; i++) { //This loop loads and prints out the card values from the array as well as loading the card images
        ...
        let srcId = "pcard" + i
        document.getElementById(srcId).src = "src/img/" + playerHand[i] + ".png"
    }
    ...
    if (isDealerTurn === true) { //Only render all cards if it's the dealer turn. Otherwise only display first card
        for (i = 0; i < dealerHand.length; i++) {
            ...
            let srcId = "dcard" + i
            document.getElementById(srcId).src = "src/img/" + dealerHand[i] + ".png"
        }
        ...
        document.getElementById("dcard0").src = "src/img/" + dealerHand[0] + ".png"
        document.getElementById("dcard1").src = "src/img/green_back.png"
        cardBlanks = 6
...
function sumCards() {
    ...
    for (ai = 0; ai < playerHand.length; ai++) {
        let value = playerHand[ai].charAt(0);
        if (value === "A") {
            pHandAces++
        } else {
            pHandVal.push(value)
    ...
    for (ai = 0; ai < dealerHand.length; ai++) {
        let value = dealerHand[ai].charAt(0);
        if (value === "A") {
            dHandAces++
        } else {
            dHandVal.push(value)
...
function hitCard() {
    if (roundOver === false) { //Only allows button to be clicked if Round is active
        ...
        playerHand.push(cardStack[cardID]);
        cardStack.splice(cardID, 1) //Puts card in player's hand removes it from the cardStack
...
function standCard() {
    if (roundOver === false) { //Only allows button to be clicked if Round is active
        ...
        if (dealerSum < dealerDrawTo) { //checks to see whether dealerSum is less than the amount the dealer draws to
            cardID = getRandomCard() //calls random card function and returns the response to cardID
            dealerHand.push(cardStack[cardID]);
            cardStack.splice(cardID, 1) //Puts card in dealer's hand and removes it from the cardStack
            renderGame() //returns to renderGame
